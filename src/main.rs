...
async fn main() {
    ...
    let tick_buffer_kraken = Arc::new(RwLock::new(TickBuffer::<BUFF_SIZE>::new("Kraken".to_string())));
let tick_buffer_binance = Arc::new(RwLock::new(TickBuffer::<BUFF_SIZE>::new("Binance".to_string())));
let balance_buffer_kraken = Arc::new(RwLock::new(BalanceBuffer::<BUFF_SIZE>::new("Kraken".to_string())));

    //load api keys from file

    let (api_key_kraken, api_sec_kraken) = utils::api_key_man::read_api_credentials_from_file("src/config/kraken_api_key").unwrap();

    let (api_key_binance, api_sec_binance) = utils::api_key_man::read_api_credentials_from_file("src/config/binance_api_key").unwrap();
    ...
    task::spawn(async move {
        ...
        loop { 
            ...
            let fast_buff_back = match tick_buffer_binance_read.buffer.back() {
                Some(back) => back,
                None => {
                    eprintln!("Error: Could not get back element from tick_buffer_binance_read.buffer");
                    continue;
                }
            };
            ...
            let slow_buff_back = match tick_buffer_kraken_read.buffer.back() {
                Some(back) => back,
                None => {
                    eprintln!("Error: Could not get back element from tick_buffer_kraken_read.buffer");
                    continue;
                }
            ...
            let balance_buffer_back = match balance_buffer_kraken_read.buffer.back() {
                Some(back) => back,
                None => {
                    eprintln!("Error: Could not get back element from balance_buffer_kraken_read.buffer");
                    continue;
                }
};
